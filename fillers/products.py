...
#     product = edit(item[0])
#     if product_id is not None:
#         cur.execute("""update stat.products SET product = CONCAT(product,', {0}') where consultation_id = {1}""".format(product, consultation_id))
#     # nepotrebne ale len pre moje info ako ide vkladanie
#     loading((len(rows)), i)
...
#             cur.execute("""update stat.products SET product_en = '{0}' where product = '{1}'""".format(product_en, product))
...
#     category = [cat for cat in dictionary if any(marker in re.findall(r'[a-z0-9]+',product_en) for marker in dictionary[cat])]      # TODO: ? vysvetlenie ?
#     if category == []:
#         category = 'uncategorized'
#         cur.execute("""update stat.products SET category = '{0}' where product_en = '{1}'""".format(category, product_en))
#     else:
#         category = category[0]
#         cur.execute(
